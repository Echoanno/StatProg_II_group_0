...
The household categories data contain monthly observations from 2000 to 2025. However, the main analysis is restricted to the complete years from 2000 to 2024. The year 2025 is excluded from the main analysis because the available observations for 2025 are incomplete. To avoid counting several monthly observations for the same year, the household analysis uses the December observations as an annual baseline.
...
#| fig-cap: "Migration inflows, outflows and migration balance in Munich, 2000-2024."
...
#| fig-cap: "Net migration balance in Munich, 2000-2024 (3-year rolling average)."
...
Around 2018, the non-EU population surpassed the EU population and continued to grow throughout the remainder of the observation period. By 2024, non-EU populations represented the larger of the two groups.

These results suggest that Munich's demographic internationalisation has increasingly been driven by non-EU populations in recent years.
...
African and American populations also grew steadily over time. Both groups approximately doubled in size between 2000 and 2024, although their absolute population levels remained substantially lower than those of European and Asian populations.
...
The household analysis uses the category Haushalte nach Personenzahl from the Munich Open Data population dataset. The analysis focuses on the years from 2012 to 2024, because household-size data are available for this period. To avoid counting several monthly observations for the same year, the analysis uses the December observations as an annual baseline. This is appropriate because household-size categories describe the number of households at a specific point in time rather than monthly flow data. The analysis includes five household-size categories: `1 Person` `2 Personen` `3 Personen` `4 Personen` `5 Personen und mehr` The dataset household_base_filtered contains the number of households by year and household size. From this dataset, we calculate the percentage share of each household type. The resulting table is stored as household_size_baseline.
...
```{r}
#| label: fig-household-percentage-trend
#| fig-cap: "Household size shares in Munich, 2012-2024."
...
The plot provides an initial overview of Munich's household structure over time. It shows that one-person households remained the largest household category throughout the period, while the relative shares of the other household-size categories changed only moderately.

The following figure compares the percentage point change of each household-size category between 2012 and 2024. This provides a category-based comparison and helps identify which household types changed the most in relative terms.

```{r}
#| label: fig-household-share-change
#| fig-cap: "Change in household shares in Munich, 2012-2024."
...
This comparison shows whether changes in household structure are concentrated in specific household-size categories. It complements the trend plot by summarising the overall change between the first and last year of the observation period.

The following figure shows the absolute number of households by household size.

```{r}
#| label: fig-household-count-trend
#| fig-cap: "Number of households by household size in Munich, 2012-2024."

knitr::include_graphics(
  "figures/household_count_plot.png"
)
```

In absolute numbers, several household categories increased over time, especially one-person households. This suggests that the number of smaller households increased, although their percentage share did not change strongly.
...
```{r}
#| label: fig-household-grouped-trend
#| fig-cap: "Grouped household structure in Munich, 2012-2024."
...
The final report will extend the household analysis in three ways.

First, household-size categories will be analysed in terms of both absolute household numbers and relative percentage shares. This will show whether changes are mainly caused by general growth in the number of households or by changes in the overall household structure.

Second, the report will compare the development of five household-size categories: `1 Person`, `2 Personen`, `3 Personen`, `4 Personen` and `5 Personen und mehr`. This will help identify which household types changed most between 2012 and 2024.

Third, the household categories will be grouped into `1 Person`, `2 Personen` and `3+ Personen`. This broader comparison will be used to examine whether there is preliminary evidence of a shift toward smaller households, or whether smaller households were already dominant throughout the period.
