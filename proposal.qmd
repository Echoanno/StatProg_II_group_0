...
For a simpler comparison, group the household types into three categories: `1 Person`, `2 Personen` and `3+ Personen`. The grouped count data are stored in `household_grouped`, and the grouped percentage data are stored in `household_grouped_percentage`.
...
For the household structure analysis, I will filter the population dataset for `MONATSZAHL == "Haushalte nach Personenzahl"` and use the December observations from 2012 to 2024 as annual reference points. I will use `year`, `category` and `count` from `household_base_filtered` to compare the absolute number of households in the five categories `1 Person`, `2 Personen`, `3 Personen`, `4 Personen` and `5 Personen und mehr`. I will report a line plot of household counts and a summary table comparing the first and last year of the analysis period. This addresses the household research question because it shows which household types increased or decreased most clearly in absolute terms.

I will then calculate the percentage share of each household type per year using `household_size_baseline`. I will report a line plot of household-size shares over time. This step addresses the research question because it shows whether the distribution of household sizes changed, instead of only showing growth in the total number of households.

Finally, I will use `household_grouped_percentage` to compare the grouped categories `1 Person`, `2 Personen` and `3+ Personen`. This dataset contains both grouped counts and percentage shares, so it can be used to show absolute developments as well as relative changes. I will report a grouped trend plot or summary table to examine whether smaller households became more important over time.
